...
public interface EdgeRepository extends JpaRepository<Opportunity, Long> {
  ...
  *  para que funcione, que no se las coloqué.*/
     ...
     @Query("SELECT a.country, COUNT(o.id) FROM Opportunity o JOIN Account a ON o.accountId=a.id GROUP BY a.country")
     List<Object[]> findOpportunitiesByCountry();

      //Count of all CLOSED_WON Opportunities by country
      @Query("SELECT a.country, COUNT(o.id) FROM Opportunity o JOIN Account a ON o.accountId=a.id " +
     ...
     @Query("SELECT a.city, COUNT(o.id) FROM Opportunity o JOIN Account a ON o.accountId=a.id GROUP BY a.city")
     List<Object[]> findOpportunitiesByCity();

     //Count of all CLOSED_WON Opportunities by the city
     @Query("SELECT a.city, COUNT(o.id) FROM Opportunity o JOIN Account a ON o.accountId=a.id " +
     ...
     @Query("SELECT a.industry, COUNT(o.id) FROM Opportunity o JOIN Account a on o.accountId=a.id GROUP BY a.industry")
     List<Object[]> findOpportunitiesByIndustry();

     //Count of all CLOSED_WON Opportunities by industry
     @Query("SELECT a.industry, COUNT(o.id) FROM Opportunity o JOIN Account a on o.accountId=a.id " +
                  "WHERE o.status = 'CLOSED_WON' GROUP BY a.industry")
     List<Object[]> findOpportunitiesClosedWonByIndustry();
